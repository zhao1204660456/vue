...
let { zqlookStartbodyArr,zq_cookie} = require('./cookie');
let zqlookStartbody = ""
let zqlookStartbodys = []

// let zq_cookie = "uid=58769276&cookie_id=50540dc4b0952384b74b33da0afc7c24&cookie=MDAwMDAwMDAwMJCMpN-w09Wtg5-Bb36eh6CPqHualq2jmrCarWOyp5drhox-ma_OqmqXr6NthJl7mI-shMmXeqDau4StacS3o7GFooLer8_IZYN5jbGEY2Ft"
let zq_cookieArr = []
let zq_cookies = ""
...
!(async () => {
    if (typeof $request !== "undefined") {
        ...
        zq_cookie1 = cookie_id + '&device_brand=xfdg&device_id=cc7dgdsgfsz83e&device_model=1gx&device_platform=android&device_type=android&inner_version=202107261526&mi=0&openudid=cc7dgdsgfsz83e&os_api=27&os_version=bdftgsdfga&phone_network=WIFI&phone_sim=1' + '&request_time=' + time1 + '&time=' + time1;

        console.log(`--------账号看看赚上方宝箱奖励执行中--------\n`)
        for (let k = 0; k < 3; k++) {
            console.log("领宝箱中", k);
            id = k.toString()
    ...
    return new Promise((resolve) => {
        ...
        $.post(url, async (err, resp, data) => {
            try {
                ...
                const result = JSON.parse(data)
                console.log("执行结果,1已完成,0未开始",result && result.items && result.items.comtele_state);
